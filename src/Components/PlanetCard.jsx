...
const PlanetCard = (props) => {
  ...
  return (
    <div
      ...
      style={{
        background: "radial-gradient(circle, rgb(106 106 161), rgb(197, 197, 246))",
      }}
    >
      <div className="flex flex-col space-y-1.5 p-4 pb-2">
        <h3 className="text-3xl font-semibold whitespace-nowrap leading-none tracking-tight text-center">
          {name}
        </h3>
      </div>
      <div className="p-4 flex flex-col gap-2">
        <CardRow label="Diameter" value={diameter + "m"} />
        <CardRow label="Climate" value={climate} />
        <CardRow label="Terrain" value={terrain} />
        <CardRow label="Population" value={population} />
      </div>
      <div className="flex items-center p-4">
        <button
          onClick={openModal}
          className="inline-flex items-center justify-center whitespace-nowrap text-xs font-bold ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-white text-black hover:bg-red-100 h-8 rounded-md px-2"
        >
          Resident Details
        </button>
      </div>
